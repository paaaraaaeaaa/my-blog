...
실제로 내 학교 이메일 계정으로 발송해서 수신까지 직접 확인했다.
...
💡 오늘 백엔드 전환에는 이메일 말고도 Google 로그인 서명 검증 하드닝(15일차의 TODO를 갚은 것)과, 구글로 처음 오는 사람은 바로 가입시키지 않고 회원가입 화면으로 보내는 흐름 변경(`feat(auth): 구글 첫 로그인 시 바로 가입시키지 않고 회원가입 화면으로 이동`)도 같은 날 같이 들어갔다 — 그 과정에서 환영 이메일이 새 가입 흐름 기준점과 어긋나 누락됐던 것도 오늘 함께 고쳤다. 두 작업 모두 [pinder API 연동기 시리즈]({{ site.baseurl }}/projects/pinder-google-oauth/)에 더 깊게 정리했다.
...
- 실사용자 이메일(내 학교 이메일 계정)로 인증번호가 실제로 발송·수신되는 것을 확인했다 — Resend의 도메인 제약을 Gmail SMTP로 우회한 결과다.
