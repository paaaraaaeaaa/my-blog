...
마우스로 클릭하는 GUI 대신 굳이 텍스트로 명령어를 입력하는 CLI를 왜 쓰는지 궁금했는데, 이유는 단순했다. Git, Docker, AWS 같은 핵심 서버 기술이 전부 CLI 기반으로 동작하기 때문이다.
...
| 영역 | 역할 |
|---|---|
| Working Directory | 실제로 코드를 작성·수정하는 로컬 폴더 |
| Staging Area | 커밋에 포함할 파일을 임시로 올려두는 공간 |
| Repository | 커밋(버전)이 영구적으로 기록되는 공간 (Local/Remote) |
...
첫 번째는 `git commit -am` 사용 중 "Untracked files..." 에러였다. 새로 만든 파일(`screens.txt`)을 `add` 없이 바로 `-am`으로 커밋하려 했는데, `-am` 옵션은 **이미 한 번이라도 추적된 파일의 수정**에만 동작한다는 걸 몰랐다. 새 파일은 반드시 최초 1회 `git add`로 스테이징해야 한다는 걸 이 에러를 보고서야 알았다.

두 번째는 브랜치 병합 중 `CONFLICT (content): Merge conflict in ...` 에러였다. `main`과 `feature/gorilla` 양쪽에서 같은 파일(`tiger.md`)의 같은 줄을 다르게 고쳐서 발생했다. 충돌 난 파일을 열어보니 `<<<<<<< HEAD` / `=======` / `>>>>>>> feature/gorilla`로 두 버전이 나란히 표시돼 있었고, IntelliJ의 Merge Tool로 원하는 코드를 고른 뒤 다시 `git add`와 `git commit`을 해서 해결했다.

세 번째는 `git clone`으로 원격 저장소를 받은 뒤 `git log`가 먹히지 않는 문제였다. 원인은 단순히 터미널 경로가 클론된 프로젝트 폴더 **바깥**에 머물러 있었기 때문이었다. `cd [프로젝트 폴더명]`으로 `.git` 폴더가 있는 안쪽까지 들어가야 한다는 걸 다시 확인했다.
