...
|---|---|---|---|---|
| 자주 쓰는 쇼핑몰 | 상품 목록 | Grid | 16px | 4장 |
| 자주 쓰는 뉴스 포털 | 기사 카드 목록 | Flex(`flex-wrap: wrap`) | 20px | 3장 |
| 자주 쓰는 커뮤니티 | 게시글 목록 | Flex(`flex-direction: column`) | 8px | 1장(세로 목록) |

#### 막혔던 점
🔴 카드 하나를 검사해서는 `flex`/`grid` 배지가 안 보여서, 부모를 계속 타고 올라가야 했는데 어디까지 올라가야 하는지 감이 안 왔다.

#### 해결 과정
🟢 요소 패널에서 카드를 감싸는 조상을 하나씩 클릭해가며 스타일 패널에 `display: flex` 또는 `display: grid`가 붙은 태그를 찾을 때까지 위로 올라갔다.

#### 배운 점
💡 실제 사이트도 카드 목록은 대부분 Flex 아니면 Grid 둘 중 하나였고, `gap` 값은 대체로 12~24px 사이였다 — 지금까지 실습에서 써본 20px이 크게 벗어난 값이 아니었다는 걸 확인했다.
...
#### 막혔던 점
🔴 창을 아주 조금씩만 좁혀서는 언제 카드가 찌그러지기 시작하는지 지점을 정확히 못 잡았다.
