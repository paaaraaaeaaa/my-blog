...
- **정렬 지시의 방향 (부모 → 자식)**
  - 가운데 놓기 같은 정렬은 항상 정렬시키고 싶은 대상의 부모에게 지시한다. `body`도 다른 요소처럼 Flex 컨테이너로 만들어 그 안의 자식을 정렬할 수 있다.

- **CSS의 조용한 실패**
  - 브라우저는 모르는 속성 이름(오타)이나 부적절한 값을 에러 없이 그냥 무시한다. 개발자 도구 스타일 패널에서 취소선이 그어진 규칙이 바로 이렇게 무시된 규칙이다.

- **`align-items` (교차축 정렬)**
  - `justify-content`가 Flex가 늘어서는 방향(주축)을 정렬한다면, `align-items`는 그 반대 방향(교차축)을 정렬한다. 정렬 대상들의 크기가 서로 같으면 값을 바꿔도 화면이 그대로일 수 있다.

- **시맨틱 태그 (`header`/`footer`)**
  - `header`/`footer`는 원래 블록 요소라 폭을 따로 지정하지 않아도 가로 전체를 차지한다. 화면 좌우에 여백이 남는다면 태그 문제가 아니라 브라우저가 `body`에 기본으로 주는 여백인 경우가 많다.
