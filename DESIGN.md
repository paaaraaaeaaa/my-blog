...
- 배경은 깊은 우주 남색, 글자는 차분한 흰색 계열, 포인트는 **신호등 같은 하늘색 네온 하나**.
...
4. **과하지 않다.** 네온 빛(glow)은 "현재 위치"를 나타낼 때만. 그라데이션 배경, 그림자 카드, 무의미한 애니메이션 금지.
...
|---|---|---|
| `--c-bg` | `#0B1120` | 페이지 배경 |
| `--c-surface` | `#111A2E` | 패널·카드 |
| `--c-surface-2` | `#172238` | hover, 표 머리, 활성 탭 |
| `--c-code-bg` | `#0E1628` | 코드 블록 |
| `--c-line` | `#223049` | 기본 구분선 |
| `--c-line-strong` | `#33425F` | 버튼·칩 테두리 |
| `--c-text` | `#E3E8F2` | 본문, 제목 |
| `--c-text-2` | `#A7B1C6` | 보조 설명, 요약 |
| `--c-text-3` | `#76819A` | 날짜, 캡션 |
| `--c-signal` | `#6FC3FF` | **유일한 포인트 색**: 링크, 활성, 진행, 포커스 |

**카테고리 색** — 상단 탭 점, 섹션 머리, 홈 바로가기 카드 윗선에만 쓴다.

| 토큰 | 값 | 카테고리 |
|---|---|---|
| `--c-cloud` | `#6FC3FF` | Cloud |
| `--c-database` | `#B39DFF` | Database |
| `--c-projects` | `#F2A96B` | Projects |
...
| `--c-danger` | `#FF7C8B` | 🔴 에러 |
| `--c-success` | `#5FD4A0` | 🟢 해결 |
| `--c-warning` | `#F3C766` | ⚠️ 🚨 주의 |

### 2.2 글꼴

| 토큰 | 글꼴 | 용도 |
|---|---|---|
| `--font-sans` | IBM Plex Sans KR | 모든 글자 (제목·본문·UI) |
| `--font-mono` | IBM Plex Mono | 코드만 |
...
| 위젯 | 파일 | 색 파라미터 |
|---|---|---|
| 방문자·조회수 뱃지 | `index.md`, `_includes/footer/custom.html` | `left_color=172238` `right_color=2B5C86` |
| GitHub Streak | `index.md` | `ring` `fire` `currStreakLabel` = signal, 숫자 = text |
| 기여 캘린더 (ghchart) | `index.md` | `084A83` — CSS 필터로 반전되어 signal 색으로 보임 |
