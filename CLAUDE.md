...
> 🎨 **디자인 관련 작업(화면, 페이지, CSS, 글 안의 HTML)은 반드시 [`DESIGN.md`](DESIGN.md)를 먼저 읽고 따른다.** 자세한 내용은 아래 9장 참고.
...
| 이모지 | 용도 | 렌더링 색상 | 언제 사용 |
|--------|------|----------|----------|
| **💡** | 요약/핵심 정리 | 하늘색 (`--c-info`) | 글의 핵심 아이디어, "여기가 중요" |
| **🔴** | 에러/버그 주의 | 빨간색 (`--c-danger`) | "이 방법은 작동하지 않음", 피해야 할 패턴 |
| **🟢** | 해결책/성공 | 초록색 (`--c-success`) | "이렇게 수정됨", 추천하는 방법 |
| **⚠️** | 경고/주의 | 노란색 (`--c-warning`) | "주의하세요", 성능 저하, 부작용 |
| **🚨** | 긴급 주의 | 노란색 (`--c-warning`) | "절대 금지", 매우 중요한 경고 |
...
**렌더링 결과:** 각 용어가 자동으로 구분선(border-top)으로 분리되고, 용어는 포인트 색(`--c-signal`, 하늘색)으로 강조됩니다.
...
| 기능 | 위치 | 동작 |
|------|------|------|
| **이전/다음 글** | 글 하단 | 같은 카테고리 + 같은 type 내에서 날짜순 연결 (`type: practice`는 같은 `topic` 안에서만, `level_order` 기준으로 연결) |
...
| **표 가로 스크롤** | 모든 표 | 좁은 화면에서 표만 옆으로 스크롤 |
| **개념 카드** | 리스트 항목이 "- **용어**" + 하위 설명 구조 | 구분선으로 분리 + 용어 포인트 색(하늘색) 강조 |
| **Mermaid 다이어그램** | Front Matter에 `mermaid: true` + 코드 블록에 ```mermaid ``` | SVG 그림으로 자동 렌더링 (색은 디자인 토큰을 따름) |
...
## 9. 디자인 규칙 (DESIGN.md 연동)

블로그의 모든 시각 규칙은 저장소 루트의 **`DESIGN.md`**가 기준이다. Agent는 디자인이 관련된 작업 전에 `DESIGN.md`를 읽고, 아래를 지킨다.
